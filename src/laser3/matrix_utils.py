...
def do_low_rank_small_singular(weight, k, debug=False, niter=2):
    assert weight.ndim == 2
    ...
    results = torch.svd_lowrank(weight,
                                q=max_rank,  # Compute full SVD to access all singular values
                                niter=niter)

    # Reverse the singular values to prioritize smaller ones
    U, S, V = results
    U = U[:, -desired_rank:]   # Select columns corresponding to smaller singular values
    S = S[-desired_rank:]      # Select the smaller singular values
    V = V[:, -desired_rank:]   # Select rows corresponding to smaller singular values

    # Reconstruct the weight approximation using smaller singular values
