...
from laser3.abstract_laser import AbstractLaser
from laser3.matrix_utils import do_low_rank_small_singular, sorted_mat, prune, do_low_rank
import pdb
...
class GPT2Laser(AbstractLaser):
    ...
    def convert_name(name):
        ...
        if name == "k_proj":
            ...
            converted_names = ["attn.c_attn.weight", "attn.c_proj.weight"]
        elif name == "all":
    ...
    def get_edited_model(model, lname, lnum, rate, intervention="rank-reduction", logger=None, in_place=True):
        ...
        for name, param in model.named_parameters():
            modify_flag = GPT2Laser._modify_layer(name=name,
                                                  lnum_to_modify=lnum,
            ...
            if intervention == 'dropout':
                ...
                # mat_analysis = do_low_rank_small_singular(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1)
            ...
            GPT2Laser.update_model(model_edit, name, mat_analysis)
            num_update += 1

        # pdb.set_trace()
        # assert num_update > 0, f"Must update some parameters GPT2: {lnum}, {lname}"
        if num_update ==0: 
            logger.log(f"No parameters updated.")
            return model
        # if logger is not None:
