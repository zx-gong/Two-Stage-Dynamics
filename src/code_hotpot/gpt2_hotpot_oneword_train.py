...
class GPT2Experiment:
    ...
    def train(self, model, tokenizer, dataset, args, llm_name):
        ...
        num_epochs = 200  # 定义训练的epoch数量
        
        self.logger.log(f"Starting training with rate {args.rate}. "
                        f"Dataset size {dataset_size}. Batch size {args.batch_size}, Epochs {num_epochs}")
        ...
        #     answer_id = tokenizer(answer)["input_ids"]
        #     if len(answer_id)==1:
        #         d = dict()
        #         d['question']=question
        #         d['answer']=answer
        ...
        for epoch in range(num_epochs+1):
            ...
            for batch_idx in tqdm(range(num_batches)):
                ...
                for i in range(batch_start, batch_end):
                    ...
                    prompted_questions_answer = prompted_question + " " + answer
                    prompted_questions_answers.append(prompted_questions_answer)
                    gold_answer_token_ids.append(answer_id)
                    
                inputs = tokenizer(prompted_questions, return_tensors="pt", padding=True, truncation=True).to(self.device)
                input_and_answers = tokenizer(prompted_questions_answers, return_tensors="pt", padding=True, truncation=True).to(self.device)
                ...
                for batch_i in range(batch_end - batch_start):
                    ...
                    predictions_ = {
                        ...
                        "prompted_question": prompted_questions[batch_i],
                        "gold-answer": answers[batch_i],
                        "generation": top_k_tokens[0].split("The answer is ", 1)[-1],
                        "correct": is_correct
                    }
            ...
            if epoch <= 100 and epoch % 10 == 0:
                torch.save(model.state_dict(), f"{save_dir}/gpt2_hotpot_oneword_epoch_{epoch}.pt")
            ...
            acc = 1.0 * is_correct_num / dataset_size 
            self.logger.log(f"Epoch {epoch} completed. Average training loss: {avg_train_loss}. Accuracy: {acc}")
            scheduler.step()
            
        save_dir = f"{home_dir}/{llm_name}/{args.intervention}/{args.lname}"
        torch.save(model.state_dict(), f"{save_dir}/gpt2_wiki_epoch_{epoch}.pt")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Process Arguments for experiments with GPT2 LLM on hotpot')
    ...
    parser.add_argument('--home_dir', type=str,
                        default="/home/zxgong/laser-main/data/iclr/hotpot_oneword/gpt2_results",
    ...
    dataset_util = Hotpot()      # We use the LLAMA tokenizer for consistency
