...
class GPT2Experiment:
    ...
    def train(self, model, tokenizer, dataset, args, llm_name):
        ...
        with torch.no_grad():
            ...
            torch.save(model_edit.state_dict(), f"{save_dir}/gpt2_hotpot_epoch_8_modify_all_{args.rate}.pt")
                
            # 计算并打印该epoch的平均损失
            avg_train_loss = train_loss / dataset_size
            print(f"Val completed. Average training loss: {avg_train_loss}")
            acc = 1.0 * is_correct_num / dataset_size 
            self.logger.log(f"Val completed. Average training loss: {avg_train_loss}. Accuracy: {acc}")
...
if __name__ == '__main__':
    ...
    logger = Logger(save_dir=save_dir, fname=f"{llm_name}-log-T8-{args.lnum}-{args.lname}-{args.rate}.txt")
